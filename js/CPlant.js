			...
			d.EleBody.style.opacity=0.5;
                    oSym.addTask(a||400,
                        function(g, f, e) {
                            ClearChild(e);
                            var h = $P[g];
                            h && h.FreeFreezeTime == f && (h.FreeFreezeTime=0,h.EleBody.style.opacity=(h.FreeSlowTime?0.75:1))
                        },
...
oStarfruit = InheritO(CPlants, {
  ...
  NormalAttack: function() {
    ...
    g.jinyin && Math.random() * 100 < 2 && (PlayAudio("portal"),oSym.addTask(300, function(h) {
      ...
      oSym.addTask(1,
        function(n, m, k, i) {
          ((n += 6) > 500 || (k += 3) > 350 ? (ClearChild(i), g.SpecialHit(g)) : (SetStyle(i, {
            left: (m += 6) + "px",
            top: k + "px"
          }), oSym.addTask(1, arguments.callee, [n, m, k, i])))
        },
        [160, 175, -100, EditEle(g.BulletEle1.cloneNode(false), {
            id: h
          },
          0, EDPZ)])
    }, ["StarB" + Math.random()]))
      ...
      EDPZ);
        ...
        NormalAttack: function() {
            ...
            oSym.addTask(1,
                ...
                [b, $(b), 20, a.PeaDire, a.AttackedLX, a.R, a.PeaKind, 0, a.AttackedLX - 40, oGd.$Torch])
        }
    }),
...
while (e--) {
  ...
  NormalAttack: function() {
    ...
    oSym.addTask(1,
      ...
      [b, $(b), 25, a.PeaDire, a.AttackedLX, a.R, Math.random() * 100 > 5 ? -1 : 0, 0, a.AttackedLX - 40, oGd.$Torch])
  }
}),
    oRepeater = InheritO(oPeashooter, {
		...
		NormalAttack2: function() {
            ...
            oSym.addTask(1,
                ...
                [b, $(b), 5,a.PeaDire, a.AttackedLX, a.R, a.PeaKind, 0, a.AttackedLX - 40, oGd.$Torch,Math.random()*3+3])
        },
        NormalAttack: function(a) {
            oSym.addTask(0,
                function(d, b) {
...
NormalAttack:function(a){
  ...
  O.AutoSummonBase = false;
                ...
                function(b) {
                    ...
                    if (c) {
                        ...
                        EditEle(f.childNodes[1], {
                            src:oCherryBomb.prototype.PicArr[3]+ Math.random()
                        }, {
                            width: "213px",
                            height: "196px",
                            left: "-50px",
	...
	oPumpkinHead = InheritO(CPlants, {
        ...
        HurtStatus: 0,
		NormalAttack:oCherryBomb.prototype.PrivateBirth,
        getHurt: function(e, c, b) {
            var d = this,
                f = d.id,
                a = $(f);
...
NewEle("oAttack_" + c.id, "div",
  ...
  NormalAttack: function() {
    ...
    NormalAttack: function() {
    var c = this,
      num = c.jinyin?Math.random() * 10000:0,
      BulletImg,
      a = c.id,
      d = "SSB" + Math.random(),
      b = c.AttackedLX;
...
NormalAttack2: function() {
   ...
   NormalAttack: function(a) {
	...
	return b > 0
        ...
        NormalAttack1: function() {
            var b = this,
                c = "CB" + Math.random(),
                num=b.jinyin?Math.random()*100:0,
                a = b.id;
  ...
  oSym.addTask(1,
    ...
    oBlover = InheritO(CPlants, {
        ...
        Tooltip: "能吹走所有气球和迷雾",
        Produce: '三叶草，能吹走所有的气球僵尸，也可以把雾吹散，把僵尸吹退几格<br>使用方法：<font color="#FF0000">单独使用，立即生效</font><br>精英形态：<font color="#FF0000">对全屏僵尸生效一次磁力菇的效果</font><br>特点：<font color="#FF0000">吹走屏幕上所有的气球僵尸</font><br>当三叶草五岁生日的时候，他得到了一个闪亮的生日蛋糕。他许好愿，然后深吸一口气却只吹灭了60%的蜡烛。然而他没有放弃，小时候的那次失败促使他更加努力直到现在。',
        AudioArr: ['blover'],
        InitTrigger: function() {},
    ...
    oFlowerVase = InheritO(CPlants, {
        ...
        InitImage: function(Kind, XRay) { // 初始化图片
            ...
            var selfValue = self.VaseValue || {},
                ...
                VValue = selfValue.Value || (VType == "SunNum" ? 50 : (VType == "Plants" ? oPeashooter : oZombie)); // 获取该花瓶的内部玩意
            switch (VType) {
                case "Plants": // 植物类型
