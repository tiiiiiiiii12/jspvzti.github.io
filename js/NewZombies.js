var oGargantuar = InheritO(oZombie, {
  ...
  PrivateAct: function(h) {
    ...
    if (h.jinyin) {
      var P = $(h.id);
       $(P.FumeDoor)&& (h.WalkDirection == h.check) && (
        SetStyle($(P.FumeDoor), {
    ...
    PrivateBirth:function(a) {
      ...
      z.appendChild(Ja)
		...
		a.ZX<=850&&!a.intograss&&(oSym.addTask(Math.random()*700+1800,function(a){
			$Z[a.id]&&a.beAttacked&&(a.BoomFire(a.R),a.checkBoomR(a),a.jinyin&&a.num<50&&a.canBoomR.length&&a.BoomFire(a.canBoomR[Math.floor(Math.random() * a.canBoomR.length)]),a.Boom=true,!a.jinyin||a.num<50?a.DisappearDie():ClearChild($(a.Ele.jinyinImg)))
...
!a.Boom&&a.jinyin&&SetStyle($(z.jinyinImg),{
	...
	var k = n.length;
      ...
      for (let i = 1; i <= oS.C; i++) {
        ...
        for (let j = 0; j < 4; j++) {
          let g = oGd.$[y + "_" + i + "_" + j];
          this.PZ&&g&&g.getHurt(this,3,1600*this.level)
      }
	}
    },
...
let A = oZ["getAr" + (a.PZ ? "HZ" : "Z")](a.PZ ? a.ZX - 120 : a.ZX, a.PZ ? a.ZX : a.ZX + 120, a.R),
      ...
      ClearChild($(b.Ele.FumeDoor));
      b.PrivateAct=function(){}
}
    }
	}else{
...
jianshangtime:500,
 getAid:function(a,d,c){
  ...
  Produce: '他遇到防御植物可将梯子搭在上面<br>扶梯韧性：<font color="#FF0000">中(500)</font><br>精英形态一：<font color="#FF0000">扶梯→玉米加农炮，速度变慢，每隔一段时间朝随机一颗植物发射炮弹，对该植物的3*3范围造成1600伤害，利用玉米炮碾压植物</font><br>精英形态二：<font color="#FF0000">金色扶梯，无限搭梯，自身有减伤，免疫负面效果，每隔一段时间给周围僵尸附上25%减伤</font><br>这架梯子花了他$114514。',
